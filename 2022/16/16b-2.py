...
class Path:
    ...
    def move(self, moveTo, number):
        ...
        if ((52 - min(self.length2, 26) - min(self.length1, 26) - 4) * (maxFlow + maxFlow2)) + self.score < max(maxScore, 2100):
            #print("kansloos")
            self.length1 = 26
            self.length2 = 26
...
while len(paths) > 0:
    ...
    for moveTo in neighbours:
        if path.length1 < 25:
            ...
            movePath.move(moveTo, 1)
            paths.append(movePath)
        if path.length2 < 25 and (path.currentLocation1 != path.currentLocation2 or path.length1 >= 25):
            movePath2 = copy.deepcopy(path)
            movePath2.move(moveTo, 2)
